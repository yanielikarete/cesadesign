...
class AjustesTable(tables.Table):
	...
	def render_action(self, value):
		delete = reverse_lazy('ajustes-delete-pk', args=[value])
		update = reverse_lazy('ajustes-update', args=[value])
...
class AjustesAprobadaTable(tables.Table):
	id_row = tables.Column(accessor='pk', orderable=False, visible=False,)
	...
	def render_action(self, value):
	    notas = reverse_lazy('aprobar-ajustes', args=[value])
	    td_str = '<a href="%s" title="Aprobar Ajuste" class="pull-right"><i class="fa fa-pencil-square-o"></i> Aprobar</a>' % str(notas)
	    return mark_safe(td_str)
