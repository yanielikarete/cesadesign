...
class OrdenEgresoTable(tables.Table):
	...
	def render_action(self, value):
		delete = reverse_lazy('ordenegreso-delete-pk', args=[value])
		update = reverse_lazy('ordenegreso-update', args=[value])
...
class OrdenEgresoAprobadaTable(tables.Table):
	id_row = tables.Column(accessor='pk', orderable=False, visible=False,)
	...
	def render_action(self, value):
	    notas = reverse_lazy('aprobar-orden-egreso', args=[value])
	    td_str = '<a href="%s" title="Aprobar Orden de Compra" class="pull-right"><i class="fa fa-pencil-square-o"></i> Aprobar</a>' % str(notas)
	    return mark_safe(td_str)

	class Meta:
		model = OrdenEgreso
		fields = ('id_row','codigo', 'proveedor','created_at','created_by','action')

class EgresoOrdenEgresoTable(tables.Table):
	id_row = tables.Column(accessor='pk', orderable=False, visible=False,)
	
	action = tables.Column("Acciones", accessor='pk', orderable=False, attrs={"th": {"class": "print-no"}, "td": {"class": "print-no"}})
	fields_report = ('codigo','proveedor','created_at','created_by',)
