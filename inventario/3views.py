...
def KardexListViewActual(request):
    ...
    if request.method == "POST" and request.is_ajax:
        ...
        try:
            cod_sku = Producto.objects.get(producto_id=sku)
        ...
        lcSql += "from kardex "
        lcSql += "where producto_id = " + str(cod_sku.producto_id) + " "
        lcSql += "and fecha_ingreso < '" + fecha_desde + "' "
        ...
        lcSql += "from kardex "
        lcSql += "where producto_id = " + str(cod_sku.producto_id) + " "
        lcSql += "and fecha_egreso < '" + fecha_desde + "' "
        lcSql += "and egreso = 't' "
        lcSql += "group by 1 "
        cursor.execute(lcSql)
        ...
        lcSql = "select case A.egreso when 't' then to_char(A.fecha_egreso, 'DD/MM/YYYY') else to_char(A.fecha_ingreso, 'DD/MM/YYYY') end as fecha, "
        ...
        lcSql += "where A.producto_id = " + str(cod_sku.producto_id) + " "
        lcSql += "and (A.fecha_ingreso >= '" + fecha_desde + "' "
        lcSql += "or A.fecha_egreso >= '" + fecha_desde + "') "
        lcSql += "order by 1"
        ...
        html += '<thead style="background-color: #EEEEEE"><tr><td colspan="9"><b>KARDEX DE PRODUCTOS </b>(SKU ' + str(cod_sku.codigo_producto) + ' ' + str(cod_sku.descripcion_producto.encode('utf8'))  + ' '  + ')<br>DESDE:' + str(fecha_desde) + ' HASTA: ' + str(fecha_hasta) +'</td>'
        ...
        html += '<td colspan="2"><b>COSTO PROMEDIO: <br>' + str("%2.2f" % cos_prome_ini).replace('.', ',') + '</b></td>'
        ...
        #html += '<tr><td colspan="15"><b>SKU: ' + str(cod_sku.codigo_producto.encode('utf8')) + ' ' + str(cod_sku.descripcion_producto.encode('utf8')) + '</td><tr>'

        #html += '<tr><td colspan="7"><b>--</b></td>'            # <td></td><td></td><td></td>'
        #html += '<td colspan="2"><b>SALDO ANTERIOR: ' + str("%2.2f" % sdo_anterior).replace('.',',') + '</b></td>'
        ...
        for co in rop_actual:
            ...
            html += '<tr><td><b>' + str(co[5].encode('utf8')) + '</b></td><td><b>' + str(co[2].encode('utf8')) + '</b></td><td>' + str(co[0]).format("%d %b %Y %H:%M:%S") + '</td><td>' + str(co[9].encode('utf8'))
            html += '</td><td>' + str(co[8].encode('utf8')) + '</td>' + '</td><td>' + str(co[12].encode('utf8')) + '</td><td>' + str(co[7].encode('utf8')) + '</td>'
            ...
            html += '<td><b>' + str(co[11].encode('utf8')) + '</b></td>' + '<td>' + str(conceptotrx.encode('utf8')) + '</td>'
            html += '</tr>'
