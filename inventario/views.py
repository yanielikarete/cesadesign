...
@login_required()
def KardexListView(request):
    lproductos = Producto.objects.filter(activo=True)

    return render_to_response('kardex/index.html',
                              RequestContext(request, {'productos': lproductos}))


#======================================================#
#Saldos de Inventario Nuevo
@login_required()
def SaldosListViewActual(request):

    today = datetime.date.today()
    toInv = 0
    fecha_today = today.strftime("%d/%m/%Y")

    cursor = connection.cursor()
    lcSql = "select B.codigo_producto, B.descripcion_producto, B.costo, B.unidad, "
    lcSql += "A.cantidad, C.nombre, (B.costo * A.cantidad) total_costo,  "
    lcSql += "case when B.activo = 't' then 'SI' else 'NO' end activo, "
    lcSql += "B.tipo_producto, D.descripcion, coalesce(E.unidades, 0) unidades "
    lcSql += "from producto_en_bodega A "
    lcSql += "left join producto B ON (A.producto_id = B.producto_id) "
    lcSql += "left join bodega C ON (A.bodega_id = C.id)"
    lcSql += "left join tipo_producto D on (B.tipo_producto = D.id) "
    lcSql += "left join requisiciones_pendientes E ON (A.producto_id = E.producto_id) "
    lcSql += "order by B.codigo_producto "
    #lcSql += "limit 400"

    cursor.execute(lcSql)
    psaldos = cursor.fetchall()

    if request.method == "POST" and request.is_ajax:
        fecha_desde = request.POST['fecha_desde']
        if (fecha_desde == fecha_today):
            #Imprimimos la consulta Inicial
            html = ''
            html += '<table class="table table-bordered table-asiento" id="kardex_productos">'
            html += '<thead style="background-color: #EEEEEE"><tr><td colspan="10"><b>Saldos de Inventario al Dia </b> ' + fecha_desde + ' <br> ' + '</td>'
            html += '</tr><tr><th style="text-align: center"><b>SKU</b></th><th style="text-align: left"><b>Producto</b></th><th style="text-align: left"><b>Tipo Producto</b></th><th style="text-align: left"><b>Bodega</b></th><th style="text-align: center">Unidad<b></b></th>'
            html += '<th style="text-align: center"><b>Stock</b></th><th style="text-align: center"><b>Costo Unit.</b></th>'
            html += '<th style="text-align: center"><b>Costo Total</b></th><th style="text-align: center"><b>Unds Reservadas</b></th>'
            html += '<th style="text-align: center"><b>Activo?</b></th>'
            html += '</tr></thead>'
            ...
            for row in psaldos:
                ...
                #html += '<tr><td>' + '11' + '</td><td>' + '22' + '</td>'
                html += '<tr><td>' + oSku + '</td><td align="left">' + oDet + '</td>'
                html += '<td align="left">' + oTip + '</td><td align="left">' + oBod + '</td>'
                html += '<td>' + oUni + '</td><td>' + str("%2.2f" % oStk).replace('.', ',') + '</td>'
                html += '<td>' + str("%2.2f" % oCos).replace('.', ',') + '</td><td>' + str("%2.2f" % oToC).replace('.', ',') + '</td>'
                html += '<td>' + str("%2.2f" % oRes).replace('.', ',') + '</td>'
                html += '<td>' + oEst + '</td>'
                html += '</tr>'

            html += '<tr><td colspan="10"><b> TOTAL INVENTARIO: ' + str("%2.2f" % toInv).replace('.', ',') + '</b></td></tr>'
            html += '</tbody>'
        else:
            vFecha = str(fecha_desde)

            lcSql = "SET datestyle TO dmy; "
            lcSql += "SELECT "
            lcSql += "B.codigo_producto, B.descripcion_producto, B.costo, "
            lcSql += "B.unidad, C.nombre, D.descripcion, "
            lcSql += "case when B.activo = 't' then 'SI' else 'NO' end activo, coalesce(E.unidades) unidades, "
            lcSql += "sum(case when A.egreso = 't' then (A.cantidad::float) * -1 else A.cantidad::float end) cantidad, "
            lcSql += "sum(case when A.egreso = 't' then (A.cantidad::float * A.costo) * -1 else (A.cantidad::float * A.costo) end) total_costo "
            lcSql += "from kardex A "
            lcSql += "left join producto B on (A.producto_id = B.producto_id) "
            lcSql += "left join bodega C on (A.bodega_id = C.id) "
            lcSql += "left join tipo_producto D on (B.tipo_producto = D.id) "
            lcSql += "left join requisiciones_pendientes E ON (A.producto_id = E.producto_id) "
            lcSql += "where (A.fecha_egreso::date <= '" + fecha_desde + "' or A.fecha_ingreso::date <= '" + fecha_desde + "') "
            lcSql += "group by 1, 2, 3, 4, 5, 6, 7, 8 "
            lcSql += "order by 1; "

            cursor.execute(lcSql)
            ksaldos = cursor.fetchall()
        ...
        html = ""
        return render_to_response('saldos/index.html', {'psaldos': psaldos}, RequestContext(request))
...
@login_required()
def KardexListViewActual(request):

    if request.method == "POST" and request.is_ajax:
        skuI = request.POST['inputSkuI']
        skuF = request.POST['inputSkuF']
        fecha_desde = request.POST['fecha_desde']
        fecha_hasta = request.POST['fecha_hasta']

        try:
            cod_skuI = Producto.objects.get(producto_id=skuI)
        except Producto.DoesNotExist:
            cod_skuI = None

        try:
            cod_skuF = Producto.objects.get(producto_id=skuF)
        except Producto.DoesNotExist:
            cod_skuF = None

        cursor = connection.cursor()

        lcSql = "select producto_id, codigo_producto, descripcion_producto "
        lcSql += "from producto "
        lcSql += "where activo = 't' and codigo_producto::float >= " + cod_skuI.codigo_producto + " "
        lcSql += "and codigo_producto::float <= " + cod_skuF.codigo_producto + " "
        lcSql += "order by codigo_producto "
        ...
        for row in Items:
            ...
            lcSql = "SET datestyle TO dmy; "
            lcSql += "select producto_id, sum(cantidad::float) as totaling, sum(cantidad::float * costo::float) as tcostoing "
            lcSql += "from kardex "
            lcSql += "where producto_id = " + str(cod_sku) + " "
            lcSql += "and fecha_ingreso::date < '" + fecha_desde + "' "
            lcSql += "and egreso is null "
            lcSql += "group by 1 "
            cursor.execute(lcSql)
            rop_ing = cursor.fetchall()

            for rowi in rop_ing:
                if rowi[1]:
                    sdo_total_ing = sdo_total_ing + rowi[1]
                if rowi[2]:
                    sdo_costo_ing = sdo_costo_ing + rowi[2]

            lcSql = "SET datestyle TO dmy; "
            lcSql += "select producto_id, sum(cantidad::float) as totalegr, sum(cantidad::float * costo::float) as tcostoegr "
            lcSql += "from kardex "
            lcSql += "where producto_id = " + str(cod_sku) + " "
            lcSql += "and fecha_egreso::date < '" + fecha_desde + "' "
            lcSql += "and egreso = 't' "
            lcSql += "group by 1 "
            cursor.execute(lcSql)
            rop_egr = cursor.fetchall()

            for rowe in rop_egr:
                if rowe[1]:
                    sdo_total_egr = sdo_total_egr + rowe[1]
                if rowe[2]:
                    sdo_costo_egr = sdo_costo_egr + rowe[2]

            sdo_anterior = sdo_total_ing - sdo_total_egr
            sdo_costoant = sdo_costo_ing - sdo_costo_egr

            if sdo_anterior > 0:
                cos_prome_ini = sdo_costoant / sdo_anterior

            lcSql = "SET datestyle TO dmy; "
            lcSql += "select case A.egreso when 't' then to_char(A.fecha_egreso, 'YYYY/MM/DD') else to_char(A.fecha_ingreso, 'YYYY/MM/DD') end as fecha, "
            lcSql += "case A.egreso when 't' then 'E' else 'I' end as tipo, "
            lcSql += "A.nro_documento, A.producto_id, A.cantidad, A.descripcion, A.costo, A.un_doc_soporte, "
            lcSql += "B.descripcion_producto, B.codigo_producto, "
            lcSql += "A.bodega_id, C.nombre, B.unidad, "
            lcSql += "case A.egreso when 't' then 1 else 0 end as orden "
            lcSql += "from kardex A "
            lcSql += "left join producto B on (A.producto_id = B.producto_id) "
            lcSql += "left join bodega C on (A.bodega_id = C.id) "
            lcSql += "where A.producto_id = " + str(cod_sku) + " "
            lcSql += "and (A.fecha_ingreso::date >= '" + fecha_desde + "' "
            lcSql += "or A.fecha_egreso::date >= '" + fecha_desde + "') "
            lcSql += "order by fecha, orden"

            cursor.execute(lcSql)
            rop_actual = cursor.fetchall()

            html += '<thead style="background-color: #EEEEEE"><tr><td colspan="9"><b>Producto </b>(SKU: ' + icodigo + ' | ' + nom_sku + ' ' + ')<br>DESDE:' + str(fecha_desde) + ' HASTA: ' + str(fecha_hasta) + '</td>'
            html += '<td colspan="2"><b>SALDO ANTERIOR: <br>' + str("%2.2f" % sdo_anterior).replace('.', ',') + '</b></td>'
            html += '<td colspan="2"><b>COSTO TOTAL: <br>' + str("%2.2f" % sdo_costoant).replace('.', ',') + '</b></td>'
            html += '<td colspan="2"><b>COSTO PROMEDIO: <br>' + str("%2.2f" % cos_prome_ini).replace('.',
                                                                                                     ',') + '</b></td>'
            html += '</tr><tr><th style="text-align: center"><b>Tipo Mov</b></th><th style="text-align: center"><b>Documento</b></th><th style="text-align: center"><b>Fecha</b></th><th style="text-align: center"><b>SKU</b></th><th style="text-align: center"><b>Producto</b></th>'
            html += '<th style="text-align: center"><b>Unidad</b></th><th style="text-align: center"><b>Glosa</b></th>'
            html += '<th style="text-align: center"><b>Cantidad</b></th><th style="text-align: center"><b>Costo Uni.</b></th><th style="text-align: center">Costo Trx.</th><th style="text-align: center"><b>Saldo Unds</b></th>'
            html += '<th style="text-align: center"><b>CosPro Unit</b></th><th style="text-align: center"><b>Saldo USD</b></th>'
            html += '<th style="text-align: center"><b>Bodega</b></th><th style="text-align: center"><b>Concepto</b></th>'
            html += '</tr></thead>'
            html += '<tbody>'
            html += '<tr><td colspan="15"> </td><tr>'

            nuevo_sdo = sdo_anterior
            for co in rop_actual:
                ...
                if tipo == "I":
                    nuevo_sdo = nuevo_sdo + cantidad
                    sdo_costoant = sdo_costoant + (costouni * cantidad)
                    conceptotrx = 'Ingreso'
                else:
                    nuevo_sdo = nuevo_sdo - cantidad
                    sdo_costoant = sdo_costoant - (costouni * cantidad)
                ...
                costotrx = costouni * cantidad

                if nuevo_sdo > 0:
                    cos_prome_ini = sdo_costoant / nuevo_sdo
                ...
                html += '<tr><td align="left"><b>' + oTipM + '</b></td><td><b>' + oDocu + '</b></td><td>' + str(co[0]).format("%d %b %Y %H:%M:%S") + '</td><td>' + icodigo
                html += '</td><td align="left">' + nom_sku + '</td>' + '</td><td>' + oUnid + '</td><td align="left">' + oGlos + '</td>'
                html += '<td align="right">' + str("%2.2f" % cantidad).replace('.', ',') + '</td>'
                html += '<td align="right">' + str("%2.2f" % costouni).replace('.', ',') + '</td>'
                html += '<td align="right">' + str("%2.2f" % costotrx).replace('.', ',') + '</td>'
                html += '<td align="right">' + str("%2.2f" % nuevo_sdo).replace('.', ',') + '</td>'
                html += '<td align="right">' + str("%2.2f" % cos_prome_ini).replace('.', ',') + '</td>'
                html += '<td align="right">' + str("%2.2f" % sdo_costoant).replace('.', ',') + '</td>'
                html += '<td><b>' + oBode + '</b></td>' + '<td>' + conceptotrx.encode('utf8') + '</td>'
                html += '</tr>'

            html += '<tr><td colspan="7"><b>--</b></td><td colspan="2"><b> SALDO FINAL: ' +str("%2.2f" % nuevo_sdo).replace('.',',') + '</b></td>'
            html += '<td colspan="2"><b> COSTO TOTAL: ' + str("%2.2f" % sdo_costoant).replace('.',',') + '</b></td> '
            html += '<td colspan="2"><b> COSTO PROMEDIO: ' + str("%2.2f" % cos_prome_ini).replace('.',',') + ' </b></td>'
...
#Saldos por Bodega
@login_required()
def SaldosListView(request):
    cursor = connection.cursor()
    lcSql = "select B.codigo_producto, B.descripcion_producto, B.costo, B.unidad, "
    lcSql += "A.cantidad, C.nombre, (B.costo * A.cantidad) total_costo,  "
    lcSql += "case when B.activo = 't' then 'SI' else 'NO' end activo, "
    lcSql += "B.tipo_producto, D.descripcion "
    lcSql += "from producto_en_bodega A "
    lcSql += "left join producto B ON (A.producto_id = B.producto_id) "
    lcSql += "left join bodega C ON (A.bodega_id = C.id)"
    lcSql += "left join tipo_producto D on (B.tipo_producto = D.id) "
