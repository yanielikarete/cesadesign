            ...
            for pe in ped:
                ...
                if pe.orden_produccion_receta:
                    html += ' <td> '+ str(pe.orden_produccion_receta.suborden_produccion.orden_produccion.tipo.encode('utf8')) +'-' + str(pe.orden_produccion_receta.suborden_produccion.orden_produccion.codigo.encode('utf8')) + ' </td>'
                     ...
                     html += '<td></td>'
                #html += ' <td> ' + DetArea + ' </td>'
                can = round(pe.cantidad, 2)
                pco = round(pe.precio_compra,2)
                tot = round(pe.total, 2)
                html += ' <td> ' + DetArea.encode('utf8') + ' </td>'
                html += ' <td> ' + str(pe.producto.codigo_producto.encode('utf8')) + ' </td>'
                html += ' <td> ' + str(pe.producto.descripcion_producto.encode('utf8')) + ' </td>'
                html += '<td>' + str(can) + '</td>'
                #html += '<td>'+str(pe.precio_compra)+'</td><td>'+str(pe.total)+'</td>'
                html += '<td>'+str(pco)+'</td><td>'+str(tot)+'</td>'
                ...
                if cont == 1:
                    ...
                    html += '<td style="vertical-align:middle" rowspan="'+str(ped.count())+'">' + str(par) + '</td>'
                    html += '<td style="vertical-align:middle" rowspan="'+str(ped.count())+'">' + str(imp) + '</td>'
                    html += '<td style="vertical-align:middle" rowspan="' + str(ped.count()) + '">' + str(sub) + '</td>'

                    #subtotal_t = subtotal_t + p[10]
                    #iva_t = iva_t + p[11]
                    #total_t= total_t + p[12]

                    subtotal_t = subtotal_t + par
                    iva_t = iva_t + imp
                    total_t = total_t + sub

                html += '</tr>'
                #total = total + round(pe.total, 2)
                #cantidad = cantidad + pe.cantidad
                #costo = costo + pe.precio_compra
...
def obtenerReporteStatusEntregas(request):
    if request.method == 'POST':
        ...
        try:
            ...
            print('pedido_data: ', pedido_data)

            # return HttpResponse(json.dumps({'status_pedidos': [pedido_data], 'guias': [guias_data]}),
            #     content_type='application/json')
            
            return render(request, 'ordenproduccion/reporteStatusEntregas.html', {'status_pedidos': [pedido_data], 'guias': [guias_data]})
